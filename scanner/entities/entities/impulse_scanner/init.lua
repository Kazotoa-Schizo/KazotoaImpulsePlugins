...
util_AddNetworkString("impulseScannerFlash")
...
function ENT:ejectPilot()
    ...
    pilot.impulseScn = nil
    pilot:SendLua([[hidden = false]])
...
function ENT:setPilot(ply)
    ...
    ply.oldWeapons = {}
    ply.oldAmmo = ply:GetAmmo()

    for k, v in pairs(ply:GetWeapons()) do
        ply.oldWeapons[k] = v:GetClass()
    end

    ply:SetRPName("DISPATCH")
    ply:StripWeapons()
    ply:StripAmmo()
...
function ENT:createFlashSprite()
    ...
    self.flashSprite = ents_Create("env_sprite")
...
function ENT:enableSpotlight()
    ...
    self.spotlight = ents_Create("point_spotlight")
    ...
    self.flashlight = ents_Create("env_projectedtexture")
...
function ENT:flash()
    ...
    net_Start("impulseScannerFlash")
        net_WriteEntity(self)
    net_SendPVS(self:GetPos())

    timer_Create("ScannerFlash"..self:EntIndex(), 0, max, function()
    ...
    self:emitDelayedSound(table_Random(self.scanSounds), 1)
end

function ENT:emitDelayedSound(source, delay, volume, pitch)
    timer_Simple(delay or 0, function()
...
function ENT:updateNoiseVelocity()
    self.noise.z = math_sin(CurTime() * 2) * 75 * (1 - self.accelZ)
end

function ENT:facePilotDirection()
    self.faceAngles = self:GetPilot():EyeAngles()
    self.faceAngles.p = math_Clamp(self.faceAngles.p, -30, 25)
end
...
function ENT:createDamageSmoke()
    ...
    self.smoke = ents_Create("env_smoketrail")
    self.smoke:SetParent(self)
    self.smoke:SetLocalPos(Vector(0, 0, 0))
...
function ENT:doDamageSound()
    ...
    elseif ((self.nextPainSound or 0) < CurTime()) then
        ...
        local painSound = table_Random(self.painSounds)
